...
def main():
    ...
    # --- NORMALIZAÇÃO FINAL DO SIDE (regra oficial) ---
    def _normalizar_side_final(rows):
        for r in rows:
            try:
                z = r.get("zona")
                g = r.get("ganho_pct")
                preco = r.get("preco")
                alvo  = r.get("alvo")
                if (z == "ONLINE") and (g is not None) and (float(g) >= GANHO_MINIMO_PCT) and (preco not in (None,0)) and (alvo not in (None,0)):
                    r["side"] = "LONG" if float(alvo) > float(preco) else "SHORT"
                else:
                    r["side"] = "NÃO ENTRAR"
            except Exception:
                r["side"] = "NÃO ENTRAR"

    _normalizar_side_final(rows)
